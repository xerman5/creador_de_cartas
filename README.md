...
«Nuevo…» abre un asistente que acompaña paso a paso hasta tener el mazo hecho. Empieza por **lo que ya tienes** y deduce de ello todo lo posible (tú solo confirmas); después viene la estructura y, con tus cartas de verdad a la vista, el diseño. Los pasos van en cuatro fases.

Cuatro principios lo guían: (1) **el asistente primero**: acompaña hasta un proyecto casi terminado; (2) **todo se ajusta después** con detalle en el editor; (3) **se vuelve al asistente** en cualquier punto para retomar algo; (4) **deducir antes que preguntar**: lo que tu material ya dice (tipos, cantidades, textos, atributos, idiomas) llega respondido.

**Empezar**

1. **Tu juego**: nombre, tamaño de carta e idiomas.
2. **Tu material**: suelta una carpeta, imágenes o una hoja de cálculo (o elígelas), o sigue sin nada y todo usa ejemplos. Son tres formas de trabajar y se pueden mezclar:
   - **Imágenes con nombre** (`elfos-ataque-001.png`, `lugar003.png`): los tipos, sus clases y sus cartas salen de los nombres (ver la convención de nombres, más abajo); con `(ref)`, son referencias.
   - **Imágenes a lo bruto** (`IMG_2041.jpg`): quedan **sin carta**. Se eligen (una a una o por subcarpeta: `lugares/` propone el tipo Lugar) y «Hacer N cartas» crea una carta de cada una en un tipo nuevo o existente, rellenando primero las cartas del tipo que no tienen imagen; el nombre del archivo puede ser el título (`guardian-de-ceniza.jpg` → «Guardian de ceniza»; los nombres de cámara no cuentan). O se dejan para ponerlas en la tabla.
   - **Una hoja de cálculo** (CSV; desde Excel, «Guardar como» CSV): además de sus cartas, crea los **tipos** que no existen (con `clase` y `subclase` si las trae), añade los **idiomas** de sus columnas (`titulo-en`), los **atributos** (una columna de números), las **habilidades** (una columna de `x`), la **rareza o facción** con sus valores, y marca en cada tipo los elementos que usan sus cartas (reglas, ambientación, coste…). Reconoce `nombre`, `texto`, `imagen`, `clan`… Nunca quita nada.

   Las imágenes se emparejan solas por el **id** de la carta, por **clase + tipo + número** y por el **título** (el de la hoja de cálculo, por ejemplo), sin pisar lo elegido a mano. Se copian a `assets/ilustraciones/`; las referencias, a `assets/referencias/`.

**Estructura**

3. **Tipos de carta** y cuántas de cada uno (ya rellenos con lo que dice tu material: solo hay que confirmar). Opcionalmente, cada tipo tiene una **clase** (Elfo, Orco…) y el tipo es su **subclase** (Ataque, Recurso…): cada combinación («Elfo · Ataque») es su propia plantilla. «＋ Otra clase con los mismos tipos» copia las subclases a una clase nueva.
4. **Qué lleva cada carta**: título (está por defecto; sin él, su banda deja el sitio al resto y la carta sigue teniendo nombre en la tabla y en los archivos), ilustración, línea de tipo, reglas, ambientación, coste, atributos y habilidades, rareza/clan/facción, número de colección. Un tipo puede ser «igual que» otro.
5. **Atributos y rareza**: cada atributo es **con número** (Ataque 3) o **solo icono** (Volar: una habilidad que la carta tiene o no); su orden en la carta (↑ ↓), cuáles lleva cada tipo (tabla de casillas) y su **icono**: se sueltan los PNG/SVG (o una carpeta) en «Tus iconos» y los que se llaman como un atributo se ponen solos (`volar.png` → Volar); los demás se arrastran al atributo o se eligen con un clic. Sin icono propio se usa uno provisional de su color. También el icono del coste y los valores de la rareza, clan o facción.
6. **Cartas**: la tabla de cada tipo, una fila por carta, con la carta seleccionada dibujada al lado. Las habilidades son casillas (las cartas que aún no se han tocado llevan algunas de ejemplo). Se puede **descargar un CSV para rellenar** (una columna por atributo; las habilidades con `x`; «;» para Excel) e **importar un CSV** (ese mismo, el del proyecto o uno propio: reconoce `nombre`, `texto`, `imagen`, `clan`…; deduce lo que dice, como en «Tu material»). Encima, la **bandeja** de las imágenes sin carta: se pulsa una para ponerla en la carta seleccionada o se arrastra a una fila; «Emparejar otra vez» vuelve a buscar por id, tipo y número y título.

**Diseño**

7. **Diseño**: cuatro diseños base (clásico, ilustración completa, retrato, texto) dibujados con tu contenido.
8. **Ajustes**: paleta, tipografía (cuatro combinaciones o **tus fuentes**: se sueltan TTF/OTF/WOFF y se eligen para títulos y textos), tamaño de la ilustración, lado de los atributos, esquina del coste, esquinas redondeadas.
9. **Tipo a tipo**: recorre cada tipo elemento por elemento, con ese elemento resaltado en la carta. Cada cambio vale para **todos los tipos** o **solo para ese tipo** (los ajustes propios se marcan con • y se pueden quitar). Lo que no se toca se queda como en el diseño; «Saltar el resto del recorrido» sigue adelante.
...
10. **Traseras**: una para todas, una por tipo o ninguna; el **dibujo del dorso** (del estante «Fondos»), la banda (color, transparencia, borde) y sus textos.
...
Desde la estructura, **«Terminar ya»** salta a «Crear» con el diseño por defecto (si no queda nada por resolver): el 90 % del mazo en un par de minutos. Se puede volver a cualquier paso después.
...
- En «Tu material» se suelta la carpeta de ilustraciones: los **tipos, sus clases y cuántas cartas tiene cada uno salen de los nombres** («Crear «Elfo · Recurso» con 3 cartas», «Hacer 3 cartas de Lugar», o «Hacerlo todo»). En un nombre de varias palabras, la primera es la **clase** (el grupo) y el resto el **tipo** (el subgrupo): `clan-energy-1` → Clan · Energy, y más tarde `clan-militar-1` → Clan · Militar, en el mismo grupo. Si el nombre es una frase (`carta-de-evento`, con «de», «la», «y»…), es todo el nombre del tipo. El material se puede traer en varias tandas: cada carpeta se suma a las anteriores. Da igual singular o plural. Un tipo mal escrito (`enemgo1.png`) se detecta y se renombra al bueno.
...
4. **Tabla**: el CSV editable dentro de la app, una fila por carta, con la carta seleccionada dibujada al lado. Filtrar por tipo y buscar; añadir, duplicar y quitar cartas (el id nuevo sigue la numeración del tipo: `lugar-004`); añadir columnas. En el panel de la carta, los **atributos** con casillas (sin número = habilidad) y el **encuadre** de cada imagen. Se guarda con ⌘S (conserva cabeceras, separador y saltos de línea del archivo) y se deshace con ⌘Z. Un id repetido se marca en rojo.
