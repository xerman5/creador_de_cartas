...
async function walk(entry: FileSystemEntry, out: File[]) {
  if (entry.isFile) {
    const f = await new Promise<File>((ok, err) => (entry as FileSystemFileEntry).file(ok, err));
    if (RESOURCE_FILE.test(f.name)) out.push(f);
...
export async function droppedFiles(e: DragEvent): Promise<File[]> {
  ...
  if (!entries.length) return [...dt.files].filter((f) => RESOURCE_FILE.test(f.name));
  const out: File[] = [];
