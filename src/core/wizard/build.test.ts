...
describe('buildProject', () => {
  ...
  it('genera filas con ids únicos, textos en cada idioma y rareza cíclica', async () => {
    ...
    expect(ids.slice(0, 2)).toEqual(['criatura001', 'criatura002']);
    expect(ids).toContain('hechizo001');
    expect(ids).toContain('criatura-epica001');
...
describe('datos de cartas y ajuste fino', () => {
  it('usa lo escrito en cada carta y rellena el resto con ejemplos', async () => {
    ...
    expect(lobo).toMatchObject({ id: 'criatura002', titulo: 'Lobo', descripcion: 'Escribe aquí el texto de reglas.', ilustracion: '' });
